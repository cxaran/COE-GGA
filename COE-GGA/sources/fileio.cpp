...
void readConfigurationFile(string fileName, ConfigurationCOE& config) {
    ...
    // Lee cada par�metro del archivo y lo asigna a la estructura Configuration
...
Instance readInstanceFile(string fileName) {
    ...
    // Lee el peso de cada art�culo y lo agrega a un nuevo objeto Item en el vector items de la instancia
    ...
    for (int i = 0; i < instance.numItems; i++) {
        ...
        for (int k = 0; k < instance.numGroups; k++) {
            ...
            // Verificaci�n si los pesos son enteros o flotantes, weightFactor ayuda a evitar errores por p�rdida de precisi�n" o "truncamiento"
    ...
    // Encontramos los � objetos grandes
