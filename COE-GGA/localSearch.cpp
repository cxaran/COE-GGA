...
bool DeleteItemToGroup(Group& group,Item& item) {
    // Encontrar objeto dentro del grupo
    for (const auto& i : group.items) if (i->id == item.id) group.items.erase(&item);
		...
		for (int j = 0; j < chromosome.groups[i].items.size(); j++)
		{
			for (int k = 1; k < chromosome.groups.size(); k++)
			{
				//Checa si representa una mejoria mover el trabajo a otra maquina
                    bool move_or_not_to_move = checkMoveSpan(chromosome.groups[i],
                                                            chromosome.groups[(i+1) % num_m],
                                                            chromosome.groups[i].items[j]);
			...
			if(moved==true){
				if(done_list[chromosome.groups[i]]==false){
                   done_list[chromosome.groups[i]]=true;
				}
			}
			break;
			}
